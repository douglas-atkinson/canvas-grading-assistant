...
Usage:
    python module_5_1_prepare_ai_copy.py \
...
def normalize_for_path_comparison(value: str) -> str:
    """Return lowercase alphanumeric text for path-identity checks."""
    return "".join(
        character.casefold()
...
def path_warnings(
    ...
    """
    Flag known student identifiers in a relative path.

    The original ZIP stem is intentionally excluded. A generic archive such
    as CarProgram.zip commonly expands into CarProgram/, so treating the ZIP
    stem itself as PII creates false positives. Student-name and Canvas-ID
    checks remain active. Path comparison also ignores punctuation and spacing
    so names such as Melissa Bott, Melissa_Bott, and MelissaBott are matched.
    """
    path_text = str(relative_path).casefold()
    normalized_path = normalize_for_path_comparison(str(relative_path))
    warnings = []

    for category, value in identifiers:
        if category == "original_zip_stem":
            continue

        raw_match = value.casefold() in path_text
        normalized_value = normalize_for_path_comparison(value)
        normalized_match = (
            len(normalized_value) >= 4
            and normalized_value in normalized_path
        )
...
def write_preview(
    ...
    parts = [
        "Canvas Exploration Module 5.1 - Outbound Preview",
        "",
